...
func (bot *Bot) processRegistration(update *tgbotapi.Update, registrationState *storage.RegistrationState) error {
	...
	case storage.RegistrationStepZero:
		...
		if err := storage.SaveRegistrationState(registrationState); err != nil {
			...
			return bot.sendMessageToUpdate(update, esc("Не удалось сохранить данные :("))
		}
		return bot.sendMessageToUpdate(update, esc("Начинаем регистрацию на контест. Введите Ваше имя:"))

	case storage.RegistrationStepName:
		name := strings.TrimSpace(update.Message.Text)
		name = trimString(name, 100)
		if len(name) == 0 {
			return bot.sendMessageToUpdate(update, esc("Попробуйте ввести имя еще раз"))
		}
		...
		return bot.sendMessageToUpdate(update, esc("Введите название Вашей школы или ВУЗа, а также класс (или курс и группу):"))

	case storage.RegistrationStepSchool:
		school := strings.TrimSpace(update.Message.Text)
		school = trimString(school, 200)
		if len(school) == 0 {
			return bot.sendMessageToUpdate(update, esc("Попробуйте ввести название образовательной организации еще раз"))
		}
		...
		return bot.sendMessageToUpdate(update, esc("Введите Ваши контактные данные (номер телефона и адрес электронной почты):"))

	case storage.RegistrationStepContacts:
		contacts := strings.TrimSpace(update.Message.Text)
		contacts = trimString(contacts, 100)
		if len(contacts) == 0 {
			return bot.sendMessageToUpdate(update, esc("Попробуйте ввести контакты еще раз"))
		}
		...
		return bot.sendMessageToUpdate(update, esc("И последний вопрос, какие предпочитаете языки и среды программирования:"))
	...
	default:
		...
		return bot.sendMessageToUpdate(update, esc("Ошибка регистрации :("))
	}
}
